...
import { getChangeInfo, getEntry, adjustTasksPositions, debug, IEditor, checkNestedTasks } from 'utils';
...
export default class MoveCheckedLinesToBottomPlugin extends Plugin {
	async onload() {
		this.app.workspace.on('editor-change', (editor: Editor, info: MarkdownView) => {
			debug('Event: editor-change', info.getMode());

			const activeView = this.app.workspace.getActiveViewOfType(MarkdownView);
			const activeEditor = activeView?.editor;

			if (activeEditor) {
				this.moveCheckedLinesToBottom(activeEditor);
			}
		});
	...
	private moveCheckedLinesToBottom(activeEditor: Editor) {
		...
		// First check nested tasks
		const contentAfterNested = checkNestedTasks(activeEditor);
		if (contentAfterNested !== activeEditor.getValue()) {
			activeEditor.setValue(contentAfterNested);
		}

		// Then handle moving tasks
